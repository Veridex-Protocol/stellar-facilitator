...
Section 3.4 is implemented by [`scheme_upto_stellar.md`](scheme_upto_stellar.md) and the stateless Soroban contract at [`contracts/upto-settlement/src/lib.rs`](../../contracts/upto-settlement/src/lib.rs). The older `upto_escrow.rs` text below is a historical proposal, not the deployed implementation.
