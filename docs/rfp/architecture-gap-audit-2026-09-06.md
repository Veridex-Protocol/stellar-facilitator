...
| Signed federation, owner/delegate authority | Full signed catalog deltas, revision/digest ordering, owner/delegate maps, revoke/restore, GossipSub | Three-process/three-database lifecycle, restart retention artifact, and deterministic tests | Announcement replay cache is memory-only; independent operators are unproven | **IMPLEMENTED + LOCAL PROCESS PROOF**; do not claim multi-operator maturity |
...
| Provider-quality loop | Signed outcomes, credential-derived sources, per-call aggregate deduplication, cross-source disagreement, Wilson aggregates, seller policy | Bazaar/SDK tests; migration `006`; prior seller flow | No diverse independent-observer corpus | **IMPLEMENTED + TESTED**; public-scale maturity remains open |
...
| Prometheus observability | Facilitator/Bazaar expose payment/RPC/channel/catalog/search/provider/P2P signals; MCP exposes per-tool signals; structured Facilitator/MCP logs correlate safe identifiers | Endpoint/operation tests and docs | External retention/alerts and unavailable upstream fee amount remain | **IMPLEMENTED + TESTED** for observable repository signals |
| Failure-domain policy is proven | Outbox replay, RPC scenarios, channel quarantine, and multi-process P2P restart behavior are automated | Focused tests plus fresh six-migration conformance/load/loopback/federation drills | Timed revalidation and real ambiguity drills remain | **PARTIAL PROOF** with explicit open drills |
...
5. Define HA outbox/quarantine ownership and run federation across independent operators.