...
| P2P equivocation/conflict | Converge deterministically | Equal revision uses canonical digest ordering | Signed full snapshots | Catalog-delta tests and three-process federation artifact | Protected for local convergence; operator policy remains local |
| Unauthorized federation signer | Reject | Owner signer or configured delegate required; transport identity is not authority | Stellar signature and authorization map | Three-node P2P test and three-process rejection vector | Protected locally |
...
| Provider evidence source confusion | Prevent caller escalation, preserve source and surface disagreement | Source derives from separate credentials; observer signer is allowlisted; one signed call contributes one aggregate sample; raw disagreements remain | Provider route and observation store | Source-escalation, observer, deduplication, store, and SDK tests | Protected locally; diverse observer evidence pending |
